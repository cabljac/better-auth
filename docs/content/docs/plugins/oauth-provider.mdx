...
oauthProvider({
  ...
  customIdTokenClaims: (user, scopes, { organization_id, metadata }) => {
    return {
      locale: "en-GB",
      "https://example.com/org": organization_id,
    };
  },
  // Attach claims to access tokens
  customJwtTokenClaims: (user, scopes, { organization_id, metadata }) => {
    return {
      "https://example.com/org": organization_id,
...
OAuth Clients are tied to either a user or organization at registration and is immutable. Thus, ensure that the [`activeOrganizationId`](/docs/plugins/organization#active-organization) is set on your active session when you create new clients.
...
- **`customUserInfoClaims`** (previously `getAdditionalUserInfoClaim`) replaces the client object parameter with the passed jwt access token instead
